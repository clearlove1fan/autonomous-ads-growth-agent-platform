...
def build_configured_outbox_store(settings: Settings) -> OutboxStore:
    if settings.outbox_backend == "none":
        return NoopOutboxStore()
    if settings.outbox_backend == "postgres":
        return PostgresOutboxStore(
            _engine_for_url(settings.database_url),
            tenant_id=settings.tenant_id,
        )
    raise ValueError(f"Unsupported outbox backend: {settings.outbox_backend}")


def dispose_cached_outbox_store_engines() -> None:
    with _ENGINE_LOCK:
