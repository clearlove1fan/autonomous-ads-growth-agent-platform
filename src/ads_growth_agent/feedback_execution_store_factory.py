...
def build_configured_feedback_execution_store(
    settings: Settings,
) -> FeedbackExecutionDryRunStore:
    if settings.feedback_execution_persistence_backend == "none":
        return NoopFeedbackExecutionDryRunStore()
    if settings.feedback_execution_persistence_backend == "postgres":
        return PostgresFeedbackExecutionDryRunStore(
            _engine_for_url(settings.database_url),
            tenant_id=settings.tenant_id,
        )
    raise ValueError(
        "Unsupported feedback execution persistence backend: "
        f"{settings.feedback_execution_persistence_backend}"
    )


def dispose_cached_feedback_execution_store_engines() -> None:
    with _ENGINE_LOCK:
