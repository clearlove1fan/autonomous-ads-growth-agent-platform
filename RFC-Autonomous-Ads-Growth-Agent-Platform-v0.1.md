...
| FR-7 | Support event-driven re-analysis | P1 | A campaign performance event can trigger analysis and revised recommendations, persisted feedback events can be listed for audit by advertiser, run, campaign, or draft, draft-only action plans and optimization drafts can be reviewed, human review decisions can be recorded, `needs_revision` reviews can produce revision drafts and second-pass reviews, individual and filtered review lineage can be inspected by event, advertiser, draft, decision, or lineage stage with execution/dry-run audit, approved dry-run execution plans can be validated and persisted through draft-only tools, operator feedback loop summaries, timelines, and command centers can show current stage, next actions, ordered audit milestones, concrete API/CLI affordances, outcome reports, and follow-up optimization re-entry commands for regressed or mixed outcomes, manual handoff packages can be generated for approved dry-run-validated changes, manual handoff outcomes can be recorded for audit, and learned memory can be retrieved in a later strategy run |
...
| CampaignFeedbackLoopCommandCenterResponse | Feedback Loop Layer | API caller, CLI user | Read-only command-center projection for one performance event, including stage-aware primary command, outcome-aware follow-up re-entry affordances, summary, and timeline |
...
| Feedback Loop | Campaign performance event ingestion, deterministic feedback analysis, draft-only action plans, draft-only optimization proposals, persisted human review decisions, revision drafts, second-pass revision reviews, individual and filtered review lineage with execution/dry-run audit, dry-run execution plans, persisted execution dry-run validation, operator feedback loop summaries, timelines, outcome-aware command centers with follow-up optimization re-entry, outcome reports, manual handoff packages, handoff outcome records, and audit reads |
...
| 2026-05-21 | Add follow-up loop re-entry commands | Regressed or mixed outcomes now link operators directly to the follow-up event action plan, optimization draft, and review command | Accepted |
