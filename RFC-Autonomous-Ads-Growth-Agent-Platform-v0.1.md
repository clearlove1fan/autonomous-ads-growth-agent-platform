...
| FR-7 | Support event-driven re-analysis | P1 | A campaign performance event can trigger analysis and revised recommendations, persisted feedback events can be listed for audit by advertiser, run, campaign, or draft, draft-only action plans and optimization drafts can be reviewed, human review decisions can be recorded, `needs_revision` reviews can produce revision drafts and second-pass reviews, individual and filtered review lineage can be inspected by event, advertiser, draft, decision, or lineage stage with execution/dry-run audit, approved dry-run execution plans can be validated and persisted through draft-only tools, operator feedback loop summaries, timelines, and command centers can show current stage, next actions, ordered audit milestones, and concrete API/CLI affordances, manual handoff packages can be generated for approved dry-run-validated changes, manual handoff outcomes can be recorded for audit, and learned memory can be retrieved in a later strategy run |
...
|---|---|---|
| Experience/API Layer | Accept advertiser requests, return strategy output, expose run, event, draft, review, and memory APIs | FastAPI endpoints for strategy generation, run detail, retry, resume, campaign performance events, feedback optimization reviews, campaign drafts, and advertiser memories; CLI for demo, eval, and debugging |
...
| CampaignFeedbackOptimizationRevisionDraftResponse | Feedback Loop Layer | API caller, CLI user | Revised draft-only changes derived from a `needs_revision` review, carrying reviewer notes back into concrete proposal changes; can be reviewed again through the existing review contract |
| CampaignFeedbackOptimizationReviewLineageResponse | Feedback Loop Layer | API caller, CLI user | Audit view linking source review, revision draft, revision reviews, execution-ready approved reviews, execution-plan summaries, and persisted dry-run validation summaries |
| CampaignFeedbackOptimizationReviewLineageListResponse | Feedback Loop Layer | API caller, CLI user | Filtered lineage audit list by event, advertiser, optimization draft, review decision, or lineage stage |
...
| CampaignFeedbackLoopTimelineResponse | Feedback Loop Layer | API caller, CLI user | Read-only ordered audit timeline for one performance event, including event analysis, action plan, optimization draft, review, revision, execution, dry-run, handoff package, and handoff outcome milestones |
| CampaignFeedbackLoopCommandCenterResponse | Feedback Loop Layer | API caller, CLI user | Read-only command-center projection for one performance event, including stage-aware primary command, available API/CLI affordances, summary, and timeline |
...
| Feedback Loop | Campaign performance event ingestion, deterministic feedback analysis, draft-only action plans, draft-only optimization proposals, persisted human review decisions, revision drafts, second-pass revision reviews, individual and filtered review lineage with execution/dry-run audit, dry-run execution plans, persisted execution dry-run validation, operator feedback loop summaries, timelines, outcome-aware command centers, outcome reports, manual handoff packages, handoff outcome records, and audit reads |
...
| Campaign performance feedback loop | Implemented | Performance snapshots produce metrics, health status, matched strategy rules from `feedback_context`, recommendations, guardrails, draft-only action plans, draft-only optimization drafts, human review records, revision drafts and second-pass reviews for `needs_revision` decisions, individual and filtered review lineage with execution/dry-run audit, dry-run execution plans, persisted execution dry-run validation through typed tools, operator feedback loop summaries, timelines, outcome-aware command centers, outcome reports, manual handoff packages, handoff outcome records, and persisted event discovery by advertiser/run/campaign/draft |
...
| Persisted product loop walkthrough | Implemented as live Postgres verifier | `python scripts/verify_persisted_product_loop.py` validates strategy draft -> feedback event -> optimization review -> revision draft -> revision review -> dry-run execution plan -> persisted execution dry-run validation -> review lineage and filtered lineage list with execution audit -> feedback loop summary, timeline, and outcome-aware command center -> manual handoff package -> handoff outcome record -> follow-up performance snapshot -> outcome report -> performance and handoff outbox memories -> API/CLI reads -> later RAG retrieval |
...
| Campaign feedback | Verify CTR/CVR/CPA/ROAS calculation, health status selection, recommendation generation, action-plan generation, optimization-draft generation, human review recording, revision-draft generation and second-pass review for `needs_revision`, review-lineage detail/list reads with execution/dry-run audit, feedback-loop summary, timeline, and command-center reads, handoff package reads, handoff outcome record reads/lists, dry-run execution-plan generation, execution dry-run validation through the typed tool registry, persisted dry-run reads/lists, and draft-only guardrails |
...
| Demo readiness | End-to-end workflow runs with seeded sample advertiser cases | Ready for deterministic local MVP demo; curated positive/negative verifiers, persisted product-loop verifier, draft-only action-plan, optimization-draft, review, revision-draft, revision-review, review-lineage detail/list with execution audit, feedback-loop summary, timeline, and command center, handoff package, handoff outcome record, execution-plan, persisted execution dry-run reads, and expected output excerpts are available |
...
| 2026-05-20 | Add feedback revision draft flow | `needs_revision` review records now produce deterministic revised draft proposals through API/CLI before another approval pass | Accepted |
| 2026-05-20 | Add feedback revision second review flow | Revision drafts can be reviewed again through API/CLI, and approved revision reviews reuse the existing execution-plan path | Accepted |
...
| 2026-05-21 | Add feedback loop timeline projection | Operators can inspect ordered event-rooted audit milestones from performance analysis through review, revision, dry-run validation, handoff package, and handoff outcome | Accepted |
| 2026-05-21 | Add feedback loop command center | Operators can inspect stage-aware primary commands and concrete API/CLI affordances for the next feedback-loop action | Accepted |
| 2026-05-21 | Add feedback outcome report | Operators can compare a feedback event with the next performance snapshot and classify improved, regressed, mixed, insufficient-data, or no-follow-up outcomes | Accepted |
...
| 2026-05-20 | Add feedback execution dry-run persistence | Dry-run validation results are optionally persisted in PostgreSQL and can be retrieved or listed for audit through API/CLI | Accepted |
