...
| FR-7 | Support event-driven re-analysis | P1 | A campaign performance event can trigger analysis and revised recommendations, persisted feedback events can be listed for audit by advertiser, run, campaign, or draft, draft-only action plans and optimization drafts can be reviewed, human review decisions can be recorded, `needs_revision` reviews can produce revision drafts and second-pass reviews, individual and filtered review lineage can be inspected by event, advertiser, draft, decision, or lineage stage with execution/dry-run audit, approved dry-run execution plans can be validated and persisted through draft-only tools, operator feedback loop summaries, timelines, command centers, and chain views can show current stage, next actions, ordered audit milestones, concrete API/CLI affordances, outcome reports, follow-up loop status, and follow-up optimization re-entry commands for regressed or mixed outcomes, manual handoff packages can be generated for approved dry-run-validated changes, manual handoff outcomes can be recorded for audit, durable outbox side effects can be listed, inspected, retried, and processed through API/CLI, and learned memory can be retrieved in a later strategy run |
...
| CampaignFeedbackLoopChainResponse | Feedback Loop Layer | API caller, CLI user | Read-only chain projection linking the baseline loop summary, outcome report, follow-up loop summary, recommended operator focus, and concrete recommended command |
...
| Feedback Loop | Campaign performance event ingestion, deterministic feedback analysis, draft-only action plans, draft-only optimization proposals, persisted human review decisions, revision drafts, second-pass revision reviews, individual and filtered review lineage with execution/dry-run audit, dry-run execution plans, persisted execution dry-run validation, operator feedback loop summaries, timelines, outcome-aware command centers with follow-up optimization re-entry, chain views, outcome reports, manual handoff packages, handoff outcome records, and audit reads |
...
| Phase 2 MVP acceptance gate | Implemented | `python scripts/verify_phase2_mvp.py` combines the live persisted product loop with external job processing, run lifecycle CLI, and ops summary checks; CI runs it in the PostgreSQL integration job |
...
| External async queue and full DLQ | Not implemented | v0.1 has pollable jobs and durable outbox side-effect visibility, but no external queue service, dead-letter topic, or replay daemon |
...
| Campaign feedback | Verify CTR/CVR/CPA/ROAS calculation, health status selection, recommendation generation, action-plan generation, optimization-draft generation, human review recording, revision-draft generation and second-pass review for `needs_revision`, review-lineage detail/list reads with execution/dry-run audit, feedback-loop summary, timeline, command-center, and chain reads, handoff package reads, handoff outcome record reads/lists, dry-run execution-plan generation, execution dry-run validation through the typed tool registry, persisted dry-run reads/lists, and draft-only guardrails |
...
| Demo readiness | End-to-end workflow runs with seeded sample advertiser cases | Ready for deterministic local MVP demo; curated positive/negative verifiers, persisted product-loop verifier, draft-only action-plan, optimization-draft, review, revision-draft, revision-review, review-lineage detail/list with execution audit, feedback-loop summary, timeline, command center, and chain view, handoff package, handoff outcome record, execution-plan, persisted execution dry-run reads, and expected output excerpts are available |
...
| 2026-05-25 | Close Phase 2 functional MVP | Phase 2 acceptance verifier now proves the local production-architecture skeleton through live Postgres product loop plus control-plane API/CLI checks | Accepted |
