...
| FR-7 | Support event-driven re-analysis | P1 | A campaign performance event can trigger analysis and revised recommendations, persisted feedback events can be listed for audit by advertiser, run, campaign, or draft, draft-only action plans and optimization drafts can be reviewed, human review decisions can be recorded, `needs_revision` reviews can produce revision drafts and second-pass reviews, individual and filtered review lineage can be inspected by event, advertiser, draft, decision, or lineage stage with execution/dry-run audit, approved dry-run execution plans can be validated and persisted through draft-only tools, operator feedback loop summaries can show current stage and next actions, manual handoff packages can be generated for approved dry-run-validated changes, manual handoff outcomes can be recorded for audit, and learned memory can be retrieved in a later strategy run |
...
| Feedback Loop Layer | Ingest campaign telemetry and return optimization recommendations | Performance event API, deterministic feedback analyzer, draft-only action plans, optimization drafts, human review records, revision drafts, second-pass revision reviews, individual and filtered review lineage with execution/dry-run audit, execution dry-run validation, operator feedback loop summary, manual handoff package, handoff outcome record, optional PostgreSQL persistence, and event-level idempotency |
...
| Feedback Loop | Campaign performance event ingestion, deterministic feedback analysis, draft-only action plans, draft-only optimization proposals, persisted human review decisions, revision drafts, second-pass revision reviews, individual and filtered review lineage with execution/dry-run audit, dry-run execution plans, persisted execution dry-run validation, operator feedback loop summaries, manual handoff packages, handoff outcome records, and audit reads |
...
| Campaign performance feedback loop | Implemented | Performance snapshots produce metrics, health status, matched strategy rules from `feedback_context`, recommendations, guardrails, draft-only action plans, draft-only optimization drafts, human review records, revision drafts and second-pass reviews for `needs_revision` decisions, individual and filtered review lineage with execution/dry-run audit, dry-run execution plans, persisted execution dry-run validation through typed tools, operator feedback loop summaries, manual handoff packages, handoff outcome records, and persisted event discovery by advertiser/run/campaign/draft |
...
| Campaign feedback | Verify CTR/CVR/CPA/ROAS calculation, health status selection, recommendation generation, action-plan generation, optimization-draft generation, human review recording, revision-draft generation and second-pass review for `needs_revision`, review-lineage detail/list reads with execution/dry-run audit, feedback-loop summary reads, handoff package reads, handoff outcome record reads/lists, dry-run execution-plan generation, execution dry-run validation through the typed tool registry, persisted dry-run reads/lists, and draft-only guardrails |
...
| Demo readiness | End-to-end workflow runs with seeded sample advertiser cases | Ready for deterministic local MVP demo; curated positive/negative verifiers, persisted product-loop verifier, draft-only action-plan, optimization-draft, review, revision-draft, revision-review, review-lineage detail/list with execution audit, feedback-loop summary, handoff package, handoff outcome record, execution-plan, persisted execution dry-run reads, and expected output excerpts are available |
