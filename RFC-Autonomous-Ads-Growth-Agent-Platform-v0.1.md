...
| FR-7 | Support event-driven re-analysis | P1 | A campaign performance event can trigger analysis and revised recommendations, persisted feedback events can be listed for audit by advertiser, run, campaign, or draft, draft-only action plans and optimization drafts can be reviewed, human review decisions can be recorded, `needs_revision` reviews can produce revision drafts and second-pass reviews, review lineage can be inspected for audit, approved dry-run execution plans can be validated and persisted through draft-only tools, and learned memory can be retrieved in a later strategy run |
...
| Feedback Loop Layer | Ingest campaign telemetry and return optimization recommendations | Performance event API, deterministic feedback analyzer, draft-only action plans, optimization drafts, human review records, revision drafts, second-pass revision reviews, review lineage, execution dry-run validation, optional PostgreSQL persistence, and event-level idempotency |
...
| CampaignFeedbackOptimizationReviewLineageResponse | Feedback Loop Layer | API caller, CLI user | Audit view linking source review, revision draft, revision reviews, and execution-ready approved reviews |
...
| Campaign performance feedback loop | Implemented | Performance snapshots produce metrics, health status, matched strategy rules from `feedback_context`, recommendations, guardrails, draft-only action plans, draft-only optimization drafts, human review records, revision drafts and second-pass reviews for `needs_revision` decisions, review lineage, dry-run execution plans, persisted execution dry-run validation through typed tools, and persisted event discovery by advertiser/run/campaign/draft |
...
| Persisted product loop walkthrough | Implemented as live Postgres verifier | `python scripts/verify_persisted_product_loop.py` validates strategy draft -> feedback event -> optimization review -> revision draft -> revision review -> review lineage -> dry-run execution plan -> persisted execution dry-run validation -> outbox memory -> API/CLI reads -> later RAG retrieval |
...
| Campaign feedback | Verify CTR/CVR/CPA/ROAS calculation, health status selection, recommendation generation, action-plan generation, optimization-draft generation, human review recording, revision-draft generation and second-pass review for `needs_revision`, review-lineage reads, dry-run execution-plan generation, execution dry-run validation through the typed tool registry, persisted dry-run reads/lists, and draft-only guardrails |
...
| Demo readiness | End-to-end workflow runs with seeded sample advertiser cases | Ready for deterministic local MVP demo; curated positive/negative verifiers, persisted product-loop verifier, draft-only action-plan, optimization-draft, review, revision-draft, revision-review, review-lineage, execution-plan, persisted execution dry-run reads, and expected output excerpts are available |
