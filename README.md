...
15. Retrieve learned advertiser memory in a later PostgreSQL-backed strategy run.
...
dry-run execution plan -> execution dry-run validation -> outbox memory ->
...
| Event-driven feedback | Campaign performance events produce health status, matched optimization rules, draft-only recommendations, action plans, optimization drafts, human review records, dry-run execution plans, and execution dry-run validation |
...
registry in dry-run mode:

```bash
curl -X POST http://localhost:8000/feedback-optimization-reviews/feedback_review_example/execution-plan/dry-run \
  -H "X-Tenant-ID: tenant_demo"

FEEDBACK_REVIEW_PERSISTENCE_BACKEND=postgres ads-growth-agent dry-run-feedback-execution-plan feedback_review_example
```
