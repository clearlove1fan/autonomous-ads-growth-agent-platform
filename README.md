...
14. Record a second-pass human review for revision drafts.
15. Inspect individual and filtered review lineage across original review, revision draft, revision review, execution readiness, and persisted dry-run validation.
...
Persisted feedback can also be turned into a concrete draft-only optimization
proposal:

```bash
curl http://localhost:8000/campaign-events/performance/evt_perf_001/optimization-draft \
  -H "X-Tenant-ID: tenant_demo"

PERFORMANCE_EVENT_PERSISTENCE_BACKEND=postgres ads-growth-agent get-feedback-optimization-draft evt_perf_001
```
...
the reviewed optimization draft. Review lineage can be inspected from either an
original review or a later revision review; when execution persistence is
enabled, the lineage also includes compact execution-plan and dry-run validation
audit summaries:

```bash
curl http://localhost:8000/feedback-optimization-reviews/feedback_review_example/lineage \
  -H "X-Tenant-ID: tenant_demo"

curl "http://localhost:8000/feedback-optimization-review-lineages?event_id=evt_perf_001&lineage_stage=revision_review&limit=20" \
  -H "X-Tenant-ID: tenant_demo"

FEEDBACK_REVIEW_PERSISTENCE_BACKEND=postgres FEEDBACK_EXECUTION_PERSISTENCE_BACKEND=postgres \
  ads-growth-agent get-feedback-optimization-review-lineage feedback_review_example
...
curl "http://localhost:8000/campaign-events/performance/evt_perf_001/feedback-loop-command-center?limit=20" \
  -H "X-Tenant-ID: tenant_demo"
...
draft-only revision proposal that carries reviewer notes into the selected
changes:
...
before execution planning or dry-run validation is allowed:

```bash
curl -X POST http://localhost:8000/feedback-optimization-reviews/feedback_review_example/revision-draft/reviews \
  -H "Content-Type: application/json" \
  -H "X-Tenant-ID: tenant_demo" \
  -d '{"decision":"approved","reviewer_id":"operator_002","notes":"Approve revised draft."}'

FEEDBACK_REVIEW_PERSISTENCE_BACKEND=postgres \
  ads-growth-agent submit-feedback-optimization-revision-review feedback_review_example --decision approved --reviewer-id operator_002
```

Approved original reviews or approved revision reviews can then be converted
into a dry-run execution plan:
...
Approved execution plans can also be validated through the internal typed tool
registry in dry-run mode. Set `FEEDBACK_EXECUTION_PERSISTENCE_BACKEND=postgres`
to persist and inspect validation results:

```bash
curl -X POST http://localhost:8000/feedback-optimization-reviews/feedback_review_example/execution-plan/dry-run \
  -H "X-Tenant-ID: tenant_demo"

FEEDBACK_REVIEW_PERSISTENCE_BACKEND=postgres FEEDBACK_EXECUTION_PERSISTENCE_BACKEND=postgres \
  ads-growth-agent dry-run-feedback-execution-plan feedback_review_example

curl http://localhost:8000/feedback-execution-dry-runs/feedback_dry_run_example \
  -H "X-Tenant-ID: tenant_demo"

curl "http://localhost:8000/feedback-execution-dry-runs?review_id=feedback_review_example&status=passed&limit=20" \
  -H "X-Tenant-ID: tenant_demo"

FEEDBACK_EXECUTION_PERSISTENCE_BACKEND=postgres ads-growth-agent get-feedback-execution-dry-run feedback_dry_run_example

FEEDBACK_EXECUTION_PERSISTENCE_BACKEND=postgres ads-growth-agent list-feedback-execution-dry-runs --review-id feedback_review_example --status passed --limit 20
```

The dry run executes only `draft_*` feedback tools, validates the tool intent
parameters with Pydantic, and returns structured step results with
`mutation_performed=false`. This proves the approved plan is executable without
...
curl -X POST http://localhost:8000/feedback-optimization-reviews/feedback_review_example/handoff-records \
  ...
  -d '{
    ...
    "notes": "Applied manually in ads workspace.",
    "completed_step_ids": ["feedback_review_example:step:1"]
  }'

curl "http://localhost:8000/feedback-handoff-records?review_id=feedback_review_example&outcome=applied" \
  -H "X-Tenant-ID: tenant_demo"

FEEDBACK_REVIEW_PERSISTENCE_BACKEND=postgres FEEDBACK_EXECUTION_PERSISTENCE_BACKEND=postgres \
  ads-growth-agent submit-feedback-handoff-record feedback_review_example \
    --outcome applied \
    --operator-id operator_001 \
    --completed-step-id feedback_review_example:step:1
